...
namespace Common.Office
{
    ...
    public class ExcelHelper
    {
        ...
        /// <param name="FirstRow">首行（标题）是否需要导处，true是要首行，flase是不要首行，默认是要首行</param>
        /// <param name="nameList"></param>
        /// <returns></returns>
        /// 
        public DataTable Excel2DataTable(string FilePath, bool FirstRow = true)
        {
            string sqlconn = "", hzm = FilePath.Substring(FilePath.LastIndexOf(".") + 1);
            hzm = hzm.ToLower();

            switch (hzm)
            {
                case "xls":
                    sqlconn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + FilePath + ";Extended Properties='Excel 8.0;HDR=no;IMEX=1'";
                    break;
                case "xlsx":
                    sqlconn = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + FilePath + ";Extended Properties='Excel 12.0;HDR=no;IMEX=1;'";
                    break;
            }
            ...
            string tableName = "";
            using (OleDbConnection conn1 = new OleDbConnection(sqlconn))
            {
                conn1.Open();
                DataTable dt = conn1.GetOleDbSchemaTable(OleDbSchemaGuid.Tables, null);
